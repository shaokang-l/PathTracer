                                 ...
                                 float tmax) const
{
  ...
  int stack_size = 0;
  stack[stack_size++] = {0, tmin, false};

  while (stack_size > 0)
  {
    StackEntry entry = stack[--stack_size];

    const auto &node = linear_bvh_nodes[entry.node_index];
    if (entry.box_tested)
    {
                                   ...
                                   float tmax, HitRecord &rec) const
{
  ...
  if (!mesh.normals.empty() && !mesh.normalIndices.empty())
  {
    ...
    gl::vec3 n0 = mesh.normals[ni[0]], n1 = mesh.normals[ni[1]], n2 = mesh.normals[ni[2]];
  ...
  if (!mesh.uvs.empty() && !mesh.uvIndices.empty())
  {
    ...
    gl::vec2 uv0 = mesh.uvs[ui[0]], uv1 = mesh.uvs[ui[1]], uv2 = mesh.uvs[ui[2]];
